...
def transformer_decoder(inputs, head_size, num_heads, ff_dim, seq_length, dropout):
    ...
    attn_output = MultiHeadAttention(
        key_dim=head_size, 
        num_heads=num_heads,
        dropout=dropout)(inputs, inputs, inputs, attention_mask=mask)
    attn_output = Dropout(dropout, training=True)(attn_output)
    ...
    ff_output = Dropout(dropout, training=True)(ff_output)
...
def create_transformer_model(vocab_size, seq_length, embed_dim=64, head_size=128, num_heads=2, ff_dim=128, dropout=0.5):
    # input sequence
...
# load data
text = load_text('../../data/nlp_train_5k.txt')
...
model = create_transformer_model(vocab_size, X0.shape[1])
model.summary()

early_stopping = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='min', restore_best_weights=True)
model.fit(X0, Y0, batch_size=64, epochs=50, validation_data=(X1, Y1))#, callbacks=[early_stopping])

model.save('../model/transformer_batch64_epoch30_seqlen60_2decoder_embedding64_headsize128_numhead3_dropout0.5_ffdim128_lr1e-3_separatepunc_5k.h5')
